...
If no PR exists and the user wants one created, derive title/body from the latest commit and capture the returned PR number into `pr_number` before any downstream section uses it:

```bash
title=$(git log -1 --pretty=format:'%s')
body=$(git log -1 --pretty=format:'%b')
pr_url=$(gh pr create --title "$title" --body "${body:-Auto-created by CodeRabbit autofix}")
pr_number=$(gh pr view "$pr_url" --json number --jq '.number')
```

Do not proceed to Section 3 or 4 until `pr_number` is populated.
...
Fail closed: a failed `gh api` call, a GraphQL `errors` payload, or a response missing `reviewThreads.pageInfo`/`nodes` must abort with an error — never fall through and treat it as "no threads found."
