...
AI-powered code review using CodeRabbit. Review-only by default: surfaces bugs, security issues, and quality risks. Do not apply fixes or modify files unless the user explicitly requests implementation, and get explicit approval for each individual edit before making it.
...
Data handling: the CLI sends code diffs to the CodeRabbit API for analysis. Before running a review, confirm the content actually being submitted does not contain secrets or credentials — this means the diff matching whatever scope is in use (uncommitted, committed, a `--base`/`--base-commit` range, or a `--dir` target), not just staged changes. If secrets are found, stop before submitting and ask the user how to proceed. Use the narrowest token scope when authenticating (`coderabbit auth login`).
...
### 4. Fix Issues (With Explicit Approval)
...
4. For each critical or warning issue, show the proposed change and get the user's explicit approval before editing — do not apply a queue of fixes unattended
